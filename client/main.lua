...
function checkFunction()
  for i, modelConfig in pairs(globalConfig.props) do
    ...
    if DoesEntityExist(table) then
      ...
      TriggerServerEvent('bsc_arm:check_sv', position)
      break
    end
  end
end

RegisterNetEvent('bsc_arm:updategrade_cl')
AddEventHandler('bsc_arm:updategrade_cl', function(gradeUpValue)

  grade = gradeUpValue

end)

RegisterNetEvent('bsc_arm:start_cl')
AddEventHandler('bsc_arm:start_cl', function()
  started = true
  if place == 1 then
    ...
    while grade >= 0.10 and grade <= 0.90 do
      ...
      if IsControlPressed(0, 38) then
        TriggerServerEvent('bsc_arm:updategrade_sv', 0.015) 
        SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
    ...
    Wait(4000)
    TriggerServerEvent('bsc_arm:disband_sv', GetEntityCoords(PlayerPedId()))
    return
...
RegisterNetEvent('bsc_arm:check_cl')
AddEventHandler('bsc_arm:check_cl', function(args)
  local table = 0
  if args == 'place1' then
    place = 1
...
RegisterNetEvent('bsc_arm:reset_cl')
AddEventHandler('bsc_arm:reset_cl', function()
  for i, modelConfig in pairs(globalConfig.props) do
